...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    rolls,sum_rolls = 0,0
    #rolls = the number that has been rolled
    flag_pig = 0
    while rolls < num_rolls:
        current_dice = dice()
        if current_dice == 1:
            flag_pig = 1
            #Turn to 1 if we have any of dice() = 1
        else:
            sum_rolls += current_dice
        rolls +=1
    if flag_pig == 0:
        return sum_rolls
    else:
...
def swine_align(player_score, opponent_score):
    ...
    def gcd_scores(x,y): #Find GCD of x,y
        gcd = 1
        i,n = 1, min(x,y)
        while i <= n:
            if x % i == 0 and y % i == 0:
    ...
    return no_zero(player_score,opponent_score) and gcd_scores(player_score,opponent_score)>=10
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def record_score(who):
        if who: #player is score1
            return opponent,score #score0,score1
        else: #player is score0
            return score,opponent #score0,score1


    def strategy_who(who):
        if who:
            return strategy1
        else:
            return strategy0
    ...
    while score0 < goal and score1 < goal:
        score, opponent = start_round(who)
        strategy = strategy_who(who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_scoring, i= 1,1
    highest_avg = 1
    this_roll = make_averaged(roll_dice, trials_count=1000)
    while i <= 10:
        avg_score = this_roll(i,dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(future_score,opponent_score):
        return 0
    else:
        if free_bacon(opponent_score) >= cutoff:
            return 0
